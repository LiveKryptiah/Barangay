/**
 * QRCode.js - Pure JavaScript QR Code Generator (SVG & Canvas)
 * Zero external dependencies. Generates crisp, scalable vector SVG or HTML5 Canvas.
...
(function(root, factory) {
      ...
      if (this.typeNumber < 1) {
        ...
        for (typeNumber = 1; typeNumber < 40; typeNumber++) {
          ...
          for (let i = 0; i < this.dataList.length; i++) {
            ...
            buffer.put(data.getLength(), 8); // 8-bit length for type 1-9
            data.write(buffer);
          }
  ...
  // High-level API for generating SVGs & Canvas
  const QRCodeGenerator = {
    // Generate an SVG element or string
    generateSVG: function(text, options = {}) {
      ...
      const level = options.correctLevel || QRErrorCorrectLevel.M;

      const qrcode = new QRCodeModel(0, level);
      qrcode.addData(text);
      qrcode.make();
      ...
      for (let row = 0; row < count; row++) {
        for (let col = 0; col < count; col++) {
          if (qrcode.isDark(row, col)) {
            rects += `<rect x="${col + margin}" y="${row + margin}" width="1" height="1" fill="${colorDark}"/>`;
          }
        }
      }
      ...
      return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${viewBoxSize} ${viewBoxSize}" width="${size}" height="${size}" shape-rendering="crispEdges">
        <rect width="${viewBoxSize}" height="${viewBoxSize}" fill="${colorLight}"/>
        ${rects}
      </svg>`;
    },

    // Render directly into a DOM container
